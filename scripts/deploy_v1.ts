...
async function main() {
	const Deployer2 = await ethers.getContractFactory("Deployer2")
	const deployer2 = (await Deployer2.deploy()) as Deployer2
	await deployer2.deployed()
	console.log("deployer to:", deployer2.address)
	...
	await deployer2.deployTransaction.wait(WAIT_BLOCK_CONFIRMATIONS)

	const badgeAddress = await deployer2.badge()
	console.log({ badgeAddress })
	// @ts-ignore
	await run(`verify:verify`, {
		address: badgeAddress,
		constructorArguments: [deployer2.address],
	})
}
