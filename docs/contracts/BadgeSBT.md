...
*The constant for the burner role (issuer)*
...
*Burner account(issuer), owner, or both can burn depending on the burn authorization.*
...
*Returns the burn authorization of the token*
...
```solidity
event MintBadge(uint8 indexed category, address indexed to, uint256 indexed tokenId)
```
