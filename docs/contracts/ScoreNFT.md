...
*Only the owner of the NFT can burn*
...
```solidity
function getScore(uint256 tokenId) external view returns (uint8 category, uint88 score, address player, address owner)
```
...
*The total number of score NFTs existing*
...
```solidity
event MintScore(uint8 indexed category, uint88 score, address indexed player, uint256 indexed tokenId)
```
